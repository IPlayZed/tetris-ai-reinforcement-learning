...
class ExtendedObservationWrapper(gym.ObservationWrapper, gym.RewardWrapper):
    ...
    def reward(self, reward):
        ...
        final_reward += 0.760066 * self.env.check_cleared_rows(self.env.board)[0]
    ...
    def custom_reward(self):
        ...
        final_penalty -= 0.510066 * np.max(heights) / 200  # Penalty for heights
        final_penalty -= 0.184483 * np.max(np.abs(bumps)) / 200  # Penalty for bumps
        final_penalty -= 0.35663 * self.env.get_holes(obs["board"]) / 200  # Penalty for holes
